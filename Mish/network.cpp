...
void Network::updateJacobian() {
  ...
  for (int l = 0; l < nLayers; ++l) {
	...
	nodes[node]->jacob[i] = (std::tanh(std::log(1.0 + std::exp(a))) + a * (std::exp(a) / (1 + std::exp(a))) / (std::cosh(std::log(1.0 + std::exp(a))) * std::cosh(std::log(1.0 + std::exp(a)))) ) * sum;
      }
    }
...
void Network::updateDeltas(const std::vector<double>& trues) {
  ...
  for (int l = hiddenLayer.size() - 1; l >= 0; --l) {
    for (int n = 0; n < hiddenLayer[l].size(); ++n) {
      ...
      sumdel            *= (std::tanh(std::log(1.0 + std::exp(a))) + a * (std::exp(a) / (1 + std::exp(a))) / (std::cosh(std::log(1.0 + std::exp(a))) * std::cosh(std::log(1.0 + std::exp(a)))) ); 
